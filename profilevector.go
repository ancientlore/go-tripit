...
func (p *ProfileVector) AppendVector(x *ProfileVector) {
	p.Vector.AppendVector(&x.Vector)
}

// At returns the i'th element of the vector.
func (p *ProfileVector) At(i int) Profile {
	return p.Vector.At(i).(Profile)
}

// Copy makes a copy of the vector and returns it.
func (p *ProfileVector) Copy() ProfileVector {
	return ProfileVector{p.Vector.Copy()}
}

// Do calls function f for each element of the vector, in order. The behavior of Do is undefined if f changes *p.
func (p *ProfileVector) Do(f func(elem Profile)) {
	p.Vector.Do(func(e interface{}) { f(e.(Profile)) })
}

// Insert inserts into the vector an element of value x before the current element at index i.
func (p *ProfileVector) Insert(i int, x Profile) {
	p.Vector.Insert(i, x)
}
...
func (p *ProfileVector) InsertVector(i int, x *ProfileVector) {
	p.Vector.InsertVector(i, &x.Vector)
}

// Last returns the element in the vector of highest index.
func (p *ProfileVector) Last() Profile {
	return p.Vector.Last().(Profile)
}

// Pop deletes the last element of the vector.
func (p *ProfileVector) Pop() Profile {
	return p.Vector.Pop().(Profile)
}

// Push appends x to the end of the vector.
func (p *ProfileVector) Push(x Profile) {
	p.Vector.Push(x)
}
...
func (p *ProfileVector) MarshalJSON() ([]byte, os.Error) {
	var a []Profile
	if p == nil {
		a = make([]Profile, 0)
	} else {
		a = make([]Profile, p.Len())
		for i := 0; i < p.Len(); i++ {
			a[i] = p.At(i)
		}
	}
...
func (p *ProfileVector) Data() []Profile {
	arr := make([]Profile, p.Len())
	var i int
	i = 0
	p.Do(func(v Profile) {
		arr[i] = v
