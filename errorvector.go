...
func (p *ErrorVector) AppendVector(x *ErrorVector) {
	p.Vector.AppendVector(&x.Vector)
}

// At returns the i'th element of the vector.
func (p *ErrorVector) At(i int) Error {
	return p.Vector.At(i).(Error)
}

// Copy makes a copy of the vector and returns it.
func (p *ErrorVector) Copy() ErrorVector {
	return ErrorVector{p.Vector.Copy()}
}

// Do calls function f for each element of the vector, in order. The behavior of Do is undefined if f changes *p.
func (p *ErrorVector) Do(f func(elem Error)) {
	p.Vector.Do(func(e interface{}) { f(e.(Error)) })
}

// Insert inserts into the vector an element of value x before the current element at index i.
func (p *ErrorVector) Insert(i int, x Error) {
	p.Vector.Insert(i, x)
}
...
func (p *ErrorVector) InsertVector(i int, x *ErrorVector) {
	p.Vector.InsertVector(i, &x.Vector)
}

// Last returns the element in the vector of highest index.
func (p *ErrorVector) Last() Error {
	return p.Vector.Last().(Error)
}

// Pop deletes the last element of the vector.
func (p *ErrorVector) Pop() Error {
	return p.Vector.Pop().(Error)
}

// Push appends x to the end of the vector.
func (p *ErrorVector) Push(x Error) {
	p.Vector.Push(x)
}
...
func (p *ErrorVector) MarshalJSON() ([]byte, os.Error) {
	var a []Error
	if p == nil {
		a = make([]Error, 0)
	} else {
...
func (p *ErrorVector) Data() []Error {
	arr := make([]Error, p.Len())
	var i int
	i = 0
	p.Do(func(v Error) {
		arr[i] = v
