...
// A specialization of Vector for *CarObject objects
type CarObjectPtrVector struct {
	vector.Vector
}

// AppendVector appends the entire vector x to the end of this vector.
func (p *CarObjectPtrVector) AppendVector(x *CarObjectPtrVector) {
	p.Vector.AppendVector(&x.Vector)
}

// At returns the i'th element of the vector.
func (p *CarObjectPtrVector) At(i int) *CarObject {
	return p.Vector.At(i).(*CarObject)
}

// Copy makes a copy of the vector and returns it.
func (p *CarObjectPtrVector) Copy() CarObjectPtrVector {
	return CarObjectPtrVector{p.Vector.Copy()}
}

// Do calls function f for each element of the vector, in order. The behavior of Do is undefined if f changes *p.
func (p *CarObjectPtrVector) Do(f func(elem *CarObject)) {
	p.Vector.Do(func(e interface{}) { f(e.(*CarObject)) })
}

// Insert inserts into the vector an element of value x before the current element at index i.
func (p *CarObjectPtrVector) Insert(i int, x *CarObject) {
	p.Vector.Insert(i, x)
}
...
func (p *CarObjectPtrVector) InsertVector(i int, x *CarObjectPtrVector) {
	p.Vector.InsertVector(i, &x.Vector)
}

// Last returns the element in the vector of highest index.
func (p *CarObjectPtrVector) Last() *CarObject {
	return p.Vector.Last().(*CarObject)
}

// Pop deletes the last element of the vector.
func (p *CarObjectPtrVector) Pop() *CarObject {
	return p.Vector.Pop().(*CarObject)
}

// Push appends x to the end of the vector.
func (p *CarObjectPtrVector) Push(x *CarObject) {
	p.Vector.Push(x)
}
...
// requested capacity.
func (p *CarObjectPtrVector) Resize(length, capacity int) *CarObjectPtrVector {
	p.Vector = *p.Vector.Resize(length, capacity)
	return p
}

// Set sets the i'th element of the vector to value x.
func (p *CarObjectPtrVector) Set(i int, x *CarObject) {
	p.Vector.Set(i, x)
}

// Slice returns a new sub-vector by slicing the old one to extract slice [i:j]. The elements are copied.
// The original vector is unchanged.
func (p *CarObjectPtrVector) Slice(i, j int) *CarObjectPtrVector {
	v := p.Vector.Slice(i, j)
	return &CarObjectPtrVector{*v}
}

// UnmarshalJSON customizes the JSON unmarshalling by accepting single elements or arrays of elements.
func (p *CarObjectPtrVector) UnmarshalJSON(b []byte) os.Error {
	var arr []*CarObject
	err := json.Unmarshal(b, &arr)
	if err != nil {
		arr = make([]*CarObject, 1)
...
func (p *CarObjectPtrVector) MarshalJSON() ([]byte, os.Error) {
	var a []*CarObject
	if p == nil {
		a = make([]*CarObject, 0)
	} else {
		a = make([]*CarObject, p.Len())
		for i := 0; i < p.Len(); i++ {
			a[i] = p.At(i)
		}
	}
...
func (p *CarObjectPtrVector) Data() []*CarObject {
	arr := make([]*CarObject, p.Len())
	var i int
	i = 0
	p.Do(func(v *CarObject) {
		arr[i] = v
		i++
	})
	return arr
