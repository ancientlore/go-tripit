...
type TravelerPtrVector struct {
	vector.Vector
}

// AppendVector appends the entire vector x to the end of this vector.
func (p *TravelerPtrVector) AppendVector(x *TravelerPtrVector) {
	p.Vector.AppendVector(&x.Vector)
}

// At returns the i'th element of the vector.
func (p *TravelerPtrVector) At(i int) *Traveler {
	return p.Vector.At(i).(*Traveler)
}

// Copy makes a copy of the vector and returns it.
func (p *TravelerPtrVector) Copy() TravelerPtrVector {
	return TravelerPtrVector{p.Vector.Copy()}
}

// Do calls function f for each element of the vector, in order. The behavior of Do is undefined if f changes *p.
func (p *TravelerPtrVector) Do(f func(elem *Traveler)) {
	p.Vector.Do(func(e interface{}) { f(e.(*Traveler)) })
}

// Insert inserts into the vector an element of value x before the current element at index i.
func (p *TravelerPtrVector) Insert(i int, x *Traveler) {
	p.Vector.Insert(i, x)
}
...
func (p *TravelerPtrVector) InsertVector(i int, x *TravelerPtrVector) {
	p.Vector.InsertVector(i, &x.Vector)
}

// Last returns the element in the vector of highest index.
func (p *TravelerPtrVector) Last() *Traveler {
	return p.Vector.Last().(*Traveler)
}

// Pop deletes the last element of the vector.
func (p *TravelerPtrVector) Pop() *Traveler {
	return p.Vector.Pop().(*Traveler)
}

// Push appends x to the end of the vector.
func (p *TravelerPtrVector) Push(x *Traveler) {
	p.Vector.Push(x)
}
...
// requested capacity.
func (p *TravelerPtrVector) Resize(length, capacity int) *TravelerPtrVector {
	p.Vector = *p.Vector.Resize(length, capacity)
	return p
}

// Set sets the i'th element of the vector to value x.
func (p *TravelerPtrVector) Set(i int, x *Traveler) {
	p.Vector.Set(i, x)
}

// Slice returns a new sub-vector by slicing the old one to extract slice [i:j]. The elements are copied.
// The original vector is unchanged.
func (p *TravelerPtrVector) Slice(i, j int) *TravelerPtrVector {
	v := p.Vector.Slice(i, j)
	return &TravelerPtrVector{*v}
}
...
func (p *TravelerPtrVector) MarshalJSON() ([]byte, os.Error) {
	var a []*Traveler
	if p == nil {
		a = make([]*Traveler, 0)
	} else {
		a = make([]*Traveler, p.Len())
		for i := 0; i < p.Len(); i++ {
			a[i] = p.At(i)
		}
	}
...
func (p *TravelerPtrVector) Data() []*Traveler {
	arr := make([]*Traveler, p.Len())
	var i int
	i = 0
	p.Do(func(v *Traveler) {
		arr[i] = v
		i++
	})
	return arr
